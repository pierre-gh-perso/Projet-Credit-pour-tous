...
def load_simple_proxy_model():
    ...
    try:
        assets = {
            "proxy_model": joblib.load(os.path.join(base_path, "chatbot_model.pkl")),
            "model_columns": joblib.load(os.path.join(base_path, "chatbot_model_columns.pkl")),
        }
        return assets
    except FileNotFoundError as e:
        st.error(f"Erreur : {e}. Entraînez le modèle.")
        return None
...
def get_loan_proposal(form_data, assets):
    ...
    else: grade, decision = "F+", "❌ Analyse approfondie requise"
